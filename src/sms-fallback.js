...
(() => {
  ...
  const copy = {
    en: {
      kicker:"LOW-CONNECTIVITY OPTION", title:"Get updates by SMS.", body:"Add your mobile number and appointment ID. CarePath will show the same synthetic visit update you would receive by SMS.", mobile:"Mobile number", id:"Appointment ID", send:"Send SMS update", clear:"Clear", sent:"SMS update prepared", demo:"Synthetic demo only — no real message is sent." , keypad:"Keypad", close:"Close"
    },
    hi: {
      kicker:"कम कनेक्टिविटी विकल्प", title:"SMS से अपडेट पाएं।", body:"अपना मोबाइल नंबर और अपॉइंटमेंट ID जोड़ें। CarePath वही सिंथेटिक विज़िट अपडेट दिखाएगा जो SMS से मिल सकता है।", mobile:"मोबाइल नंबर", id:"अपॉइंटमेंट ID", send:"SMS अपडेट भेजें", clear:"साफ़ करें", sent:"SMS अपडेट तैयार है", demo:"सिर्फ सिंथेटिक डेमो — कोई वास्तविक संदेश नहीं भेजा जाता।", keypad:"कीपैड", close:"बंद करें"
    },
    kn: {
      kicker:"ಕಡಿಮೆ ಸಂಪರ್ಕ ಆಯ್ಕೆ", title:"SMS ಮೂಲಕ ಅಪ್‌ಡೇಟ್ ಪಡೆಯಿರಿ.", body:"ನಿಮ್ಮ ಮೊಬೈಲ್ ಸಂಖ್ಯೆ ಮತ್ತು ಅಪಾಯಿಂಟ್‌ಮೆಂಟ್ ID ಸೇರಿಸಿ. SMS ಮೂಲಕ ಸಿಗುವ ಅದೇ ಸಿಂಥೆಟಿಕ್ ಭೇಟಿ ಅಪ್‌ಡೇಟ್ ಅನ್ನು CarePath ತೋರಿಸುತ್ತದೆ.", mobile:"ಮೊಬೈಲ್ ಸಂಖ್ಯೆ", id:"ಅಪಾಯಿಂಟ್‌ಮೆಂಟ್ ID", send:"SMS ಅಪ್‌ಡೇಟ್ ಕಳುಹಿಸಿ", clear:"ತೆರವುಗೊಳಿಸಿ", sent:"SMS ಅಪ್‌ಡೇಟ್ ಸಿದ್ಧವಾಗಿದೆ", demo:"ಸಿಂಥೆಟಿಕ್ ಡೆಮೋ ಮಾತ್ರ — ಯಾವುದೇ ನಿಜವಾದ ಸಂದೇಶ ಕಳುಹಿಸಲಾಗುವುದಿಲ್ಲ.", keypad:"ಕೀಪ್ಯಾಡ್", close:"ಮುಚ್ಚಿ"
    }
  };

  const stateFromRoute = () => {
    const raw = location.hash.replace(/^#\/?healthcare\/visit\//,"");
    return raw || "appointment-confirmed";
  };
  ...
  function onClick(e){
    ...
    if(key){
      const input=document.getElementById("cp-sms-mobile");
      if(!input)return;
      activeField="mobile";
      const value=input.value;
      if(key.dataset.key==="clear") input.value="";
      else if(key.dataset.key==="back") input.value=value.slice(0,-1);
      else if(value.length<10) input.value=value+key.dataset.key;
      input.focus();
      return;
    }
    ...
    if(send){
      ...
      if(!/^\d{10}$/.test(mobile)){card.dataset.status="Enter a valid 10-digit mobile number.";render();return}
      if(id!=="DEMO-042"){card.dataset.status="For this synthetic demo, use DEMO-042.";render();return}
      sessionStorage.setItem(KEY,mobile);
      card.dataset.status=messageFor(stateFromRoute());
      render();
      return;
    }
  }
  window.addEventListener("carepath:language-changed",e=>{lang=e.detail?.lang||localStorage.getItem("carepath:language:v3")||"en";render()});
  window.addEventListener("carepath:route-rendered",render);
