...
export const initialJourney = Object.freeze({
  ...
  appointment: { hospital: "City Government Hospital", department: "Orthopaedics", clinician: "Dr. Mehta", date: "22 Aug 2026", time: "10:30 AM" },
  visit: { token: "42", registrationCounter: "Counter 3", building: "OPD Block A · Ground Floor" },
...
export function applyEvent(journey, event, now = new Date()) {
  ...
  next.lastUpdated = event.at ?? formatTime(now);
  ...
  switch (event.type) {
    ...
    case EventType.CALL_PATIENT:
      ...
      next.queueAhead = 0;
      break;
    case EventType.START_CONSULTATION: next.state = JourneyState.CONSULTATION; break;
    case EventType.COMPLETE_CONSULTATION: next.state = JourneyState.LAB; next.queueAhead = null; break;
    case EventType.COMPLETE_LAB: next.state = JourneyState.PHARMACY; break;
    case EventType.COMPLETE_PHARMACY: next.state = JourneyState.COMPLETED; break;
  }

  const description = event.description ?? event.type.replaceAll("_", " ");
  next.events = [...journey.events, { type: event.type, description, at: next.lastUpdated }];
  return next;
}
